        ...
        <div className="max-w-7xl mx-auto">
            ...
            <form onSubmit={handleSearch} className="relative max-w-3xl mx-auto">
              <div className="relative">
                <input
                  ...
                  placeholder="Search for AI tools, tutorials, and resources..."
                  className="w-full px-5 py-4 pr-12 rounded-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-lg"
                />
                <button 
                  type="submit"
                  className="absolute right-2 top-1/2 transform -translate-y-1/2 p-2 text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400"
                  disabled={isLoading}
                >
                  {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 border border-gray-100 dark:border-gray-700">
                <div className="flex items-center">
                  <FontAwesomeIcon icon={faSearch} className="h-5 w-5 text-blue-500 mr-2" />
                  <div>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Total Searches</p>
                    <p className="text-xl font-semibold">{analytics.totalSearches.toLocaleString()}</p>
                  </div>
                </div>
              </div>
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 border border-gray-100 dark:border-gray-700">
                <div className="flex items-center">
                  <FontAwesomeIcon icon={faChartBar} className="h-5 w-5 text-green-500 mr-2" />
                  <div>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Cache Hit Rate</p>
                    <p className="text-xl font-semibold">{analytics.cacheHitRate}%</p>
                  </div>
                </div>
              </div>
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 border border-gray-100 dark:border-gray-700">
                <div className="flex items-center">
                  <FontAwesomeIcon icon={faSpinner} className="h-5 w-5 text-yellow-500 mr-2" />
                  <div>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Avg Response</p>
                    <p className="text-xl font-semibold">{analytics.averageResponseTime}ms</p>
                  </div>
                </div>
              </div>
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 border border-gray-100 dark:border-gray-700">
                <div className="flex items-center">
                  <FontAwesomeIcon icon={faTag} className="h-5 w-5 text-purple-500 mr-2" />
                  <div>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Popular Terms</p>
                    <p className="text-xl font-semibold truncate">{analytics.topSearchTerms[0]?.term || 'N/A'}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-4 mb-6">
            ...
            <div className="relative" ref={filtersRef}>
              ...
              {showFilters && (
                <div className="origin-top-left absolute left-0 mt-2 w-80 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 z-10">
                  <div className="p-4">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="block w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm text-gray-900 dark:text-white"
                        >
            ...
            <div className="relative" ref={sortRef}>
              <button
                ...
                className="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
              >
                <FontAwesomeIcon icon={faSort} className="h-4 w-4 mr-2" />
                Sort by: {sortOptions.find(opt => opt.value === sortConfig.by)?.label || 'Most Relevant'}
              </button>
            ...
            {!hasSearched ? (
              <div className="text-center py-12">
                <div className="text-gray-400 dark:text-gray-500 mb-4">
                  ...
                  <h2 className="text-xl font-medium">Search our AI tools and resources</h2>
                  <p className="mt-2">Enter a search term above to find what you're looking for</p>
                </div>
              </div>
            ) : isLoading ? (
              <div className="text-center py-12">
                <FontAwesomeIcon 
                  icon={faSpinner} 
                  className="h-8 w-8 text-blue-500 animate-spin mx-auto mb-4" 
                />
                <p className="text-gray-600 dark:text-gray-400">Searching for "{q}"...</p>
              </div>
            ) : error ? (
                            ...
                            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">
                              <Link href={result.url} className="hover:text-blue-600 dark:hover:text-blue-400">
                                ...
                                <FontAwesomeIcon 
                                  ...
                                  className="ml-1.5 h-3 w-3 text-gray-400 dark:text-gray-500 inline-block"
                                />
                              </Link>
                            </h3>
                            {result.snippet && (
                            ...
                            <div className="flex flex-wrap items-center gap-3 mt-2 text-xs">
                              {result.rating !== undefined && result.rating > 0 && (
                                <div className="flex items-center text-gray-600 dark:text-gray-400">
                                  <FontAwesomeIcon icon={faStar} className="h-3 w-3 text-yellow-400 mr-1" />
                              ...
                              {result.tags && result.tags.length > 0 && (
                                <div className="flex flex-wrap gap-1">
                                  {result.tags.slice(0, 3).map(tag => (
                                    <span 
                                      key={tag} 
                                      className="px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded"
                                    >
                                      {tag}
                                    </span>
                                  ))}
              ...
              <div className="text-center py-12">
                <div className="text-gray-400 dark:text-gray-500">
                  ...
                  <h2 className="text-xl font-medium">No results found for "{q}"</h2>
                  <p className="mt-2">Try different keywords or check your spelling</p>
                  
                  <div className="mt-6">
                    <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-3">Try these suggestions:</h3>
                    <div className="flex flex-wrap justify-center gap-2">
                      <button 
                        onClick={() => setSearchQuery('AI chatbot')}
                        className="px-3 py-1.5 text-sm bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full text-gray-700 dark:text-gray-300"
                      >
                        AI chatbot
                      </button>
                      <button 
